...
- [ ] **Cablear CRUD de contactos y edición de sucursal:** desde 2026-06-06 la ficha del cliente permite crear/editar/eliminar contactos (encargado único) y editar toda la info de la sucursal (zona, canal, clasificación, GPS, activa) sobre estado local. Al cablear: INSERT/UPDATE/DELETE en `contacts` (el delete debe borrar también en Supabase) y UPDATE en `stores`; garantizar encargado único en BD (constraint parcial o lógica de servidor).
