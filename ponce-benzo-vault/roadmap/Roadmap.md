...
- [x] **Nivel Cliente (cadena) + Piloto Farmatodo (2026-06-15):** nueva tabla `clients` + FK/columnas en `stores` (migración). El Excel de Farmatodo es la **fuente de verdad** del piloto: 4 cadenas; **45 tiendas** (36 act. + 9 creadas) con GPS real, dirección, zona y encargado; **76 rutas** nuevas (mercaderista + día, horizonte 4 semanas); **154 tiendas no-piloto desactivadas**; alta de Jonathan. Hub: tab **Tiendas** + filtro por cliente + página **Clientes**. Ver [[decisiones/ADR-004-Nivel-Cliente-Piloto-Farmatodo|ADR-004]], [[pendientes/Piloto Farmatodo - Reconciliacion Tiendas|Reconciliación]] y [[logs/Log-2026-06-15|Log]].
...
- [x] **Sincronización Móvil:** ✅ *login + rutas (2026-06-08)* y ✅ *motor de sync (2026-06-15)*. Cliente Supabase + login persistido + ruta del día con fallback; y la **cola SQLite→Supabase** (sessions/pings/visits/competencia) con subida de fotos a Storage, casi en vivo, idempotente, sin deps nativas (OTA). Ver [[logs/Log-2026-06-15-mobile-sync|Log Mobile Sync]] y `docs/superpowers/specs/2026-06-15-mobile-sync-design.md`. **Pendiente:** E2E en dispositivo; signed URLs en el hub para mostrar fotos; sync en background con app cerrada.
...
- [x] **Filtros en Tareas (2026-09-07):** vendedor (vía `client_assignments`), cliente, tipo y buscador combinables, más enlace directo `/panel/tareas?task=<id>` para los correos. Desplegado en producción el mismo día. Ver [[logs/Log-2026-09-07|Log 2026-09-07]].
- [ ] **Correo al vendedor al crearse una tarea:** diseñado y planificado el 2026-09-07 (`notification_log` + trigger + `pg_net` → route handler del hub → Resend). Bloqueo externo: DNS de `ponce-benzo.com`.
